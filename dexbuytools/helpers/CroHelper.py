...
from dexbuytools.helpers.data.cro import chain_data

class CroHelper(EvmBaseHelper):

    DEFAULT_DEX = 'MMF' #PancakeSwap

    def __init__(self, config, dex_name=None, custom_rpc=None):
        w3 = Web3(Web3.HTTPProvider(config.general_params['CRO_RPC_URL'] if custom_rpc is None else custom_rpc))
        w3.middleware_onion.inject(geth_poa_middleware, layer=0)

        dex_name = CroHelper.DEFAULT_DEX if dex_name is None else dex_name
